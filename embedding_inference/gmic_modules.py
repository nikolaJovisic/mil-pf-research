...
class ResNetV1(nn.Module):
    ...
    def forward(self, x):
        # first sequence
        #print("in gmic module:", x.get_device(), x.shape, flush=True)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        #print("in gmic module done:", x.get_device(), x.shape, flush=True)
...
class DownsampleNetworkResNet18V1(ResNetV1):
    ...
    def forward(self, x):
        ...
        #print("last feature map downsamplenetwork:", last_feature_map.shape)
...
class PostProcessingStandard(nn.Module):
    ...
    def forward(self, x_out):
        ...
        #    normalized_saliencymap = F.softmax(out, dim=1)
        #else:
        normalized_saliencymap = torch.sigmoid(out)
...
class RetrieveROIModule(AbstractMILUnit):
    ...
    def forward(self, x_original, cam_size, h_small):
        ...
        :param cam_size: (h, w) -> saliency map height, width
        ...
        #print(h,h_h, flush=True)
        #print(w,w_h, flush=True)
        ...
        '''if self.num_classes > 1:
            cam_flatten = current_images.view(N, C, -1)
            top_t = int(round(w*h*self.percent_t))
            print("top_t value:", top_t, flush=True)
            selected_area, _ = cam_flatten.topk(top_t, dim=2) #topt_index: N, 1, 110
            print("selected_area:", selected_area.shape, flush=True)
            print(selected_area.mean(dim=2), flush=True)
            print(selected_area.mean(dim=2).max(), flush=True)
            max_val, class_index = torch.max(selected_area.mean(dim=2))
            max_vals = current_images.view(N, class_index, -1).max(dim=2, keepdim=True)[0].unsqueeze(-1)
            min_vals = current_images.view(N, class_index, -1).min(dim=2, keepdim=True)[0].unsqueeze(-1)
            range_vals = max_vals - min_vals
            normalize_images = current_images - min_vals
            current_images = normalize_images / range_vals
        else:'''
...
class AttentionModule(AbstractMILUnit):
    ...
    def add_layers(self):
        ...
        if self.parameters['learningtype'] == 'SIL':
            '''if self.parameters["num_classes"] > 1:
                self.parent_module.classifier_linear = nn.ModuleList()
    ...
    def forward(self, h_crops):
        ...
        '''if self.parameters['num_classes'] > 1:
            ...
            attn_score_reshape = attn_score.view(batch_size, num_crops, self.parameters['num_classes'])
            attn = F.softmax(attn_score_reshape, dim=1)
            print("attn score:", attn)
        else:'''
        ...
        # final hidden vector
        '''if self.parameters['num_classes'] > 1:
            z_weighted_avg = torch.empty((batch_size, self.parameters['num_classes'], h_dim))
            for k in range(self.parameters['num_classes']):
                z_weighted_avg[:, k, :] = torch.sum(attn[:,:,k].unsqueeze(-1) * h_crops, 1)
        else:'''
        ...
        if self.parameters['learningtype'] == 'SIL':
            '''if self.parameters['num_classes'] > 1:
                y_crops = torch.empty((batch_size, self.parameters['num_classes']))
                for k in range(self.parameters['num_classes']):
