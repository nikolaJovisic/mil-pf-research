...
from multiclass_embeddings_dataset import EmbeddingsDataset

train_batch_size = 256 #float('inf') #500000 #float('inf') #200000  #2**18
valid_batch_size = 256 #float('inf')
test_batch_size = 256 #float('inf')

def get_dataset_cfg():
    embeddings_root = '/lustre/nj/cvpr2026/embeddings' 

    def get_path(split):
        return f'{embeddings_root}/vindr-density-msl/{split}/embeddings.hdf5'
    ...
    return {
        'train': ([get_path(f'train-gpu{gpu}') for gpu in range(6)], classes),
        'valid': ([get_path(f'valid-gpu{gpu}') for gpu in range(6)], classes),
        'test': ([get_path(f'test-gpu{gpu}') for gpu in range(6)], classes)
    }
...
pickle.dump(datasets, open('/lustre/nj/cvpr2026/pickles/vindr-density-msl.pkl', 'wb'))
