...
class Trex(nn.Module):
    def __init__(self, cfg):
        ...
        half_dim = cfg.input_dim // 2

        self.global_proj_0 = nn.Sequential(nn.Linear(half_dim, 2 * cfg.gl_hidden_dim), nn.ReLU())
        self.global_proj_1 = nn.Sequential(nn.Linear(2 * cfg.gl_hidden_dim, cfg.gl_hidden_dim), nn.ReLU())

        self.local_proj_0 = nn.Sequential(nn.Linear(half_dim, 2 * cfg.lc_hidden_dim), nn.ReLU())
        self.local_proj_1 = nn.Sequential(nn.Linear(2 * cfg.lc_hidden_dim, cfg.lc_hidden_dim), nn.ReLU())

        self.explora_proj = nn.Linear(half_dim, cfg.explora_proj_dim)
    ...
    def forward(self, x, group, instance_type):
        ...
        x_first, x_second = x[:, : self.cfg.input_dim // 2], x[:, self.cfg.input_dim // 2 :]
